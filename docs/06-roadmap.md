...
| # | 任务 | 包 | 说明 |
|---|---|---|---|
| 3.1 | ✅ 扩充题库 + 分类随机 | prompts | 现象场景分类（天文宇宙/冷与热/力与运动/光与看/开放追问），带答案 |
...
- ✅ 打卡墙「最近一次睡觉打卡」附加展示：状态之外显示上次睡觉时间（按打卡时区 24h），凌晨打卡标「今天凌晨」；管理员可在后台切换 `exact`（显示时间）/ `presence`（仅状态）
- ✅ 目标就寝时间可随时在打卡页修改（限 20:00-23:59）
- ✅ 睡前趣味思考题（带答案 + 领域过滤 + 抽题即打卡 + 次日历史题库）
- ✅ 手动起床（`wake_up`，提前退出睡眠状态）
- ✅ 个人数据导出 + 注销（口令核验）
