 ...
 *   逐人推送到其各自的 ntfy topic（members.notify_topic），而非共享 topic。
 * - 尊重成员的 notify_enabled：关闭通知的成员跳过。
 * - 个性化目标时间提醒（按各自目标前 N 分钟）仍留到 Phase 2，本任务不做。
...
export async function runReminder(deps: JobDeps): Promise<void> {
  ...
  for (const group of listGroups(db)) {
    ...
    const members = listMembers(db, group.id).filter(
      (m) => m.status === 'active' && m.notifyEnabled,
    );
    ...
    for (const member of members) {
      ...
      await notify.publish(member.notifyTopic, reminderMessage(member.nickname, member.targetBedtime), {
        priority: 3,
      });
      sent += 1;
    }
