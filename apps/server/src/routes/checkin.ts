...
import { getCheckin, getGroupById, upsertCheckin } from '@healthy-life/db';
...
export function checkinRoutes(deps: AppDeps): Hono<Env> {
  ...
  router.post('/checkin', (c) => {
    const member = c.get('member');
    const group = getGroupById(deps.db, member.groupId);
    if (!group) return c.json({ error: 'group not found' }, 404);

    const now = new Date();
    const date = currentCheckinDay(group.timezone, now);
    ...
    const wc = wallClock(group.timezone, now);
